...
public function alvinaThirdEncounterYes():void
{
	...
	outputText("(If Lethice dead) You even defeated Lethice for me, which I will admit is highly convenient.(end of cut)");
	...
	if (!player.hasStatusEffect(StatusEffects.PlayerPhylactery)) outputText("(if pc has a soul still) I’m amazed a being still carrying a soul would achieve what you did.");
	...
	addButtonDisabled(4, "Never", "NYI (stay tune for 0.8n2 to fight her in meantime make last will and etc. *wink wink*)");
}
public function alvinaThirdEncounterYesSure():void
{
	outputText("Learning black magic straight from the source? Where's the dark contract so you can sign it!? You shake hand with Alvina right away, sealing the deal.\n\n");
...
public function alvinaThirdEncounterYesNever():void
{
	...
	flags[kFLAGS.ALVINA_FOLLOWER] = 5;
	doNext(camp.returnToCampUseOneHour);
}
			...
			if (rand(2) == 0) outputText("\"<i>Do you think, if I tried hard enough, I could fit this chair inside me [name]?</i>\" You try not to think about this weird question as you back away.\n\n");
				...
				if (rand(2) == 0) outputText("\"<i>Various recipes for cooking lethicite mmm… perhaps I should try that one with fellblooms.</i>\" You back away before she considers using yours for it.\n\n");
					...
					if (rand(2) == 0 && date.month == 7) {
						...
						else outputText("\"<i>Did you know bee girls actually weren’t immune to corruption? I caught and used a few handmaidens as test subjects for some new spells before releasing them back to their hive. Apparently, you can create an entire army of demons just by corrupting a few and letting them spread mayhem and lust back home, very fascinating.</i>\"\n\n");
					}
				}
			}
		}
...
public function alvinaCampAdvancedStudy():void
{
	...
	if (player.statusEffectv1(StatusEffects.AlvinaTraining2) == 3 && player.hasKeyItem("Siegweird holy symbol") >= 0) {
		player.removeKeyItem("Siegweird holy symbol");
		...
		outputText("You don’t need her repeating herself and cut straight into your side wincing as you cut a fairly sized chunk, healing the wound afterward. Alvina takes the freshly cut morsel of skin and incant the organic matter sinking in the gem of the necklace where it won’t risk decaying. All base preparation is done she has you silently rehearse the line repeatedly so you don't mess up.\n\n");
		outputText("\"<i>Wouldn’t want to accidentally lose your soul because of a failed ritual right, [name]? Repeat after me…</i>\"\n\n");
		outputText("You repeat it a few times, then finally ready off for the real one. As instructed, you stand in the middle of the circle, holding the items next to your heart and start incanting after drinking the five succubus milks in one go.\n\n");
		outputText("\"<i>Ahgn akoshor fisfis arci gorshaness firnklir gorash nok arokis severis...</i>\"\n\n");
		outputText("You hold yourself back from cumming several times; knowing full well that if you do, your soul is forfeit. A ghostly thread starts to form between your chest and the necklace and as the thread stabilizes, you see something being extracted bit by bit from you in the form of white mist, which is swiftly absorbed by the chunk of flesh in the amulet. ");
		...
		if (player.devilkinScore() >= 11 || (player.demonScore() >= 14 && player.gender > 0)) {
			if (player.devilkinScore() >= 14) {
			...
			outputText("\"<i>You look demonic alright, but you still lack the power, a power only a soul-less body can provide. First, we need to create the soul prison. Get me a flawless Amethyst necklace, five lust drafts, ten soul gems, and a piece of very powerful Lethicite then we can begin.</i>\"\n\n");
			player.addStatusValue(StatusEffects.AlvinaTraining2, 1, 1);
		}
